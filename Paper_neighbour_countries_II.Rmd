...
**Monat** | **`r month(sel$time[nrow(sel)-1],label=TRUE,abbr=FALSE)`** | **`r month(sel$time[nrow(sel)-1],label=TRUE,abbr=FALSE)`**
------------- | -------------|-------------
...
#d <- filter(dat,indic %in% ty, geo == "IT", s_adj =="SA",time>="2010-01-01") 
d_hind_i <- filter(dat_hind_fac_i,indic ==  "BS-FLP3-PC", geo %in% cat, s_adj == "SA",time>="2016-01-01") 

d_hind_i$time <- as.yearqtr(d_hind_i$time)
...
#break.vec <- c(seq(from = min(d_hind_i$time), to = max(d_hind_i$time),
#                   by = "16 month"),max(d_hind_i$time))
...
p <- ggplot(data = d_hind_i) + 
    ...
    scale_y_continuous(labels = function(x) paste0(x*1, "%")) +
  #scale_x_date(date_labels = "%b %y",breaks=break.vec) +
    zoo::scale_x_yearqtr(breaks = seq(from = min(d_hind_i$time), to = max(d_hind_i$time), by = 2),format = '%Y Q%q')+
  theme(axis.text.x = element_text( 
    angle = 60, 
    #color="blue", 
    vjust=0.6,
...
d_hind_s <- filter(dat_hind_fac_s,indic =="BS-FLB3-PC", geo %in% cat, s_adj == "SA",time>="2019-01-01") 


d_hind_s$time <- as.yearqtr(d_hind_s$time)

#d_hind_s$time <- paste(format(d_hind_s$time, "%Y"),  # Convert dates to quarterly
#                            sprintf("%i", (as.POSIXlt(d_hind_s$time)$mon) %/% 3L + 1L), 
#                            sep = "-Q")


#sprintf("%i", (as.POSIXlt(d_hind_s$time %m-% months(1))$mon) %/% 3L + 1L), 
...
break.vec <- c(seq(from = as.Date(as.yearqtr(min(d_hind_s$time))), to = as.Date(as.yearqtr(max(d_hind_s$time))),
                   by = "quarter"),as.Date(as.yearqtr(max(d_hind_s$time))))
...
p <- ggplot(data = d_hind_s) + 
    ...
    theme(text = element_text(size = 15))+
    scale_y_continuous(labels = function(x) paste0(x*1, "%")) +
 # scale_x_date(date_labels = "%YQ%q",breaks=break.vec) +
 # zoo::scale_x_yearqtr(limits = c(min(d_hind_s$time), max(d_hind_s$time)),format = '%Y Q%q')+
  zoo::scale_x_yearqtr(breaks = seq(from = min(d_hind_s$time), to = max(d_hind_s$time), by = 2),format = '%Y Q%q')+
  #scale_x_yearqtr(format = "%YQ%q",breaks=break.vec )+
  theme(axis.text.x = element_text( 
    angle = 60, 
    #color="blue", 
    vjust=0.6,
...
break.vec <- c(seq(from = min(d_empl_exp_wh$time), to = max(d_empl_exp_wh$time),
                   by = "12 month"),max(d_empl_exp_wh$time))
...
break.vec <- c(seq(from = min(d_empl_exp_eco$time), to = max(d_empl_exp_eco$time),
                   by = "12 month"),max(d_empl_exp_eco$time))
...
d_cons_mood <- filter(dat_cons_mood,unit== "BAL",indic %in% t, geo %in% cat, s_adj =="SA",time>="2018-01-01")
...
break.vec <- c(seq(from = min(d_cons_mood$time), to = max(d_cons_mood$time),
                   by = "16 month"),max(d_cons_mood$time))
...
a <- as.data.frame(tsl)
b <- seq(as.Date("2009/4/1"), by = "month", length.out = nrow(a))
a <- relocate(mutate(a,b),b)

a <- a %>% 
  rename(
    time=b,
    value=ch.kof.bts_total.ng08.fx.q_ql_ass_bs.balance.d11
    )
...
p <- ggplot(data=sel,
       mapping=aes(x=time,y=value))+
    geom_line(aes(y = value, color = "Geschäftslagenindikator"),size=.7)+
    scale_color_manual(name="",values = c("#8E44AD","#ffa500","#659065","#A0522D")) +
    #  geom_text(data=dat %>% group_by(geo)%>% filter(time==max(time)),size=2.6) + 
    # theme(legend.position="left")+
    #  scale_color_manual(values = c("#8E44AD", "#F1C40F","#34495E","#138D75","#E74C3C","#F8C471"))+
...
```{r plot321, echo=FALSE,  include=FALSE, message=FALSE,warning=FALSE}
...
dat_hind_fac_i <- get_eurostat(id="ei_bsin_q_r2") #Industry (quarterly: ei_bsin_q_r2)
dat_hind_fac_s <- get_eurostat(id="ei_bsse_q_r2") #Services (quarterly: ei_bsin_q_r2)
dat_hind_constr <- get_eurostat(id="ei_bsbu_m_r2") #construction monthly


d_hind_i <- filter(dat_hind_fac_i,indic ==  "BS-FLP3-PC", geo %in% cat, s_adj == "SA",time>="2016-01-01") 
d_hind_s <- filter(dat_hind_fac_s,indic =="BS-FLB3-PC", geo %in% cat, s_adj == "SA",time>="2016-01-01")
d_hind_c <- filter(dat_hind_constr,indic =="BS-FLBA4-PC", geo %in% cat, s_adj == "SA",time>="2016-01-01") 

d_hind_i$time <- as.yearqtr(d_hind_i$time)
d_hind_s$time <- as.yearqtr(d_hind_s$time)
d_hind_c <- d_hind_c %>%
  group_by(geo,time = paste(lubridate::year(time),quarters(time))) %>%
  summarise(values = mean(values)) %>% 
  ungroup()
d_hind_con <- select(d_hind_c,time,geo,values)
d_hind_con <- arrange(d_hind_con,desc(time))
d_hind_con$time <- as.yearqtr(d_hind_con$time)

d_hind_i <- rename(d_hind_i,Industry=values)
d_hind_s <- rename(d_hind_s,Services=values)
d_hind_con <- rename(d_hind_con,Construction=values)

d_hind_i <- d_hind_i %>% 
  select(time, geo, Industry)
d_hind_s <- d_hind_s %>% 
  select(time, geo, Services)
d_hind_con <- d_hind_con %>% 
  select(time, geo, Construction)

d_hind_both <- merge(d_hind_i,d_hind_s,by=c("time","geo"),all=TRUE)
d_hind_both <- merge(d_hind_both,d_hind_con,by=c("time","geo"),all=TRUE)

d_hind_both <- pivot_longer(d_hind_both, cols = 3:5, names_to ="branches",
 values_to = "values")
  
  

d_hind_both$geo[d_hind_both$geo=="AT"] <- "Österreich"
d_hind_both$geo[d_hind_both$geo=="EA19"] <- "EU-19"
d_hind_both$geo[d_hind_both$geo=="DE"] <- "Deutschland"
d_hind_both$geo[d_hind_both$geo=="FR"] <- "Frankreich"
d_hind_both$geo[d_hind_both$geo=="IT"] <- "Italien"

d_hind_both$branches[d_hind_both$branches=="Industry"] <- "Industrie"
d_hind_both$branches[d_hind_both$branches=="Services"] <- "Dienstleistungen"
d_hind_both$branches[d_hind_both$branches=="Construction"] <- "Baugewerbe"


#d_hind_both$indic[d_hind_both$indic=="BS-FLP3-PC"] <- "Arbeitskräftemangel"
#d_hind_i$indic[d_hind_i$indic=="BS-FLP5-PC"] <- "Andere"
#d_hind_i$indic[d_hind_i$indic=="BS-FLP1-PC"] <- "Keine"
#d_hind_i$indic[d_hind_i$indic=="BS-FLP2-PC"] <- "Insufficient Demand"
...
p <- ggplot(data = d_hind_both) + 
    geom_line(mapping = aes(x = time, y = values, color=branches),size=.6) + 
    scale_color_manual(values = c("#eb4255","#8E44AD","#ffa500","#659065","#A0522D")) +
    #  geom_text(data=dat %>% group_by(geo)%>% filter(time==max(time)),size=2.6) + 
    # theme(legend.position="left")+
    #  scale_color_manual(values = c("#8E44AD", "#F1C40F","#34495E","#138D75","#E74C3C","#F8C471"))+
...
pdf('/Users/simonwey/Repos/Foreign_economics/Graphics/wrap_Hin_both.pdf')
print(p)
dev.off()
...
```{r plot2d, echo=FALSE,  include=FALSE, message=FALSE,warning=FALSE}
...
dat_hind <- get_eurostat(id="ei_bsbu_m_r2") #Industry (quarterly: ei_bsin_q_r2)

d <- filter(dat_hind,indic =="BS-FLBA4-PC", geo %in% cat, s_adj == "SA",time>="2019-03-01") 

a <- d %>%
  group_by(geo,quarter = paste(lubridate::year(time),quarters(time))) %>%
  summarise(values = mean(values)) %>% 
  ungroup()
b <- select(a,quarter,geo,values)
b <- arrange(b,desc(quarter))

b$quarter <- as.yearqtr(b$quarter)

#b <- data.frame(apply(b,2,unclass))

  
b$geo[b$geo=="AT"] <- "Österreich"
b$geo[b$geo=="EA19"] <- "EU-19"
b$geo[b$geo=="DE"] <- "Deutschland"
b$geo[b$geo=="FR"] <- "Frankreich"
b$geo[b$geo=="IT"] <- "Italien"

#d$indic[d$indic=="BS-FLBA4-PC"] <- "Arbeitskräftemangel"


break.vec <- c(seq(from = min(d$time), to = max(d$time),
                   by = "24 month"),max(d$time))

p <- ggplot(data = b) + 
    geom_line(mapping = aes(x = quarter, y = values,color=geo),size=.6) + 
    scale_color_manual(values = c("#8E44AD","#ffa500","#659065","#A0522D","red")) +
    #  geom_text(data=dat %>% group_by(geo)%>% filter(time==max(time)),size=2.6) + 
    # theme(legend.position="left")+
    #  scale_color_manual(values = c("#8E44AD", "#F1C40F","#34495E","#138D75","#E74C3C","#F8C471"))+
    labs(caption="Quelle: Eurostat", #title = "Erwerbslosenrate",
 ...
 # scale_x_date(date_labels = "%YQ%q",breaks=break.vec) +
 # zoo::scale_x_yearqtr(limits = c(min(d_hind_s$time), max(d_hind_s$time)),format = '%Y Q%q')+
    zoo::scale_x_yearqtr(breaks = seq(from = min(b$quarter), to = max(b$quarter), by = 2),format = '%Y Q%q')+
    theme(axis.text.x = element_text( 
    angle = 60, 
    #color="blue", 
    vjust=0.6,
...
pdf('/Users/simonwey/Repos/Foreign_economics/Graphics/wrap_Hind_constr_q.pdf')
print(p)
dev.off()
